...
\end{table}
		...
		\begin{tabular}{c|cccc}
			\hline
			s & \multicolumn{4}{c}{\textbf{Constraints over AR}}\\\hline
			
			 1 & 
			\scalebox{0.8}{\begin{subfigure}[t]{1.5cm}
				\centering
				\begin{tikzpicture}
					\node (bt1) at (-.4,0.8) {L};
					\node (bt2) at (.4,0.8) {H};
...
%				\vspace{em}
			...
			\scalebox{0.8}{			\begin{subfigure}[t]{1.5cm}
				\centering
				\begin{tikzpicture}
					\node (bt1) at (0.4,0.8) {H};
					\node (bt2) at (.8,0.8) {L};
					\node (bt3) at (1.2,0.8) {H};
					\node (bc1) at (-.4,0) {$\mu$};
					\node (bc2) at (0,0) {$\mu$};
					\node (bc3) at (.4,0) {$\mu$};
					\node (bs1) at (-.4,-0.8) {$\sigma$};
...
\noindent \textbf{Experiment} \newsec{\textit{Algorithm}} We ran an experiment using two versions of BUFIA \citep{chandleeLearningPartiallyOrdered2019} on the same dataset, and then compared the constraints they returned. BUFIA is a breadth-first deterministic learning algorithm that searches a partially-ordered hypothesis space from the most general constraints to more specific
